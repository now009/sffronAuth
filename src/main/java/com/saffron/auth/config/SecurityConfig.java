...
public class SecurityConfig {
    ...
    // application.yml cors.allowed-origins 목록을 주입
    @Value("${cors.allowed-origins}")
    private List<String> allowedOrigins;

    // /auth/** 경로 전용 필터 체인 (로그인 API)
    // @Order(0): Authorization Server(@Order(1))보다 먼저 적용
    @Bean
    @Order(0)
    ...
    // 기본 필터 체인 (그 외 모든 경로)
    // @Order(2): Authorization Server(@Order(1)) 다음에 적용
    @Bean
    @Order(2)
